...
configure_argocd() {
  ...
  # Set the default policy to admin to ensure anonymous users have full Web UI access
  kubectl "${KUBECTL_CONTEXT_ARGS[@]}" -n argocd patch configmap argocd-rbac-cm --type merge -p '{"data":{"policy.default": "role:admin"}}'
